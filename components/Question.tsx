                ...
                <ul className='flex space-x-3 list-none mt-1'>
                    {
                        question.tags.map(tag => (
                            <li key={tag.id} onClick={() => router.push(`/questions?tag=${encodeURIComponent(tag.tagName)}`)}><Tag tagName={tag.tagName} /></li>
                        ))
                    }
                </ul>

            </div>
            <div className='absolute right-2 bottom-1 text-sm'><span className='cursor-pointer text-blue-500 hover:text-blue-300' onClick={() => router.push(`/profile/${question.user.id}`)}>{question.user.username}</span>{` asked ${getTimeSince(question.createdTime)} ago`}</div>
        </div>
    )
}
